  ...
  const options: ChartOptions<'line'> = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        borderColor: isDark ? 'rgba(241,245,249,.13)' : 'rgba(15,23,42,.1)',
        borderWidth: 1,
        padding: 10,
        callbacks: {
          label: (ctx) =>
            ctx.datasetIndex === 0
              ? `  Sesiones: ${ctx.parsed.y.toLocaleString()}`
              : `  Ingresos: €${ctx.parsed.y.toLocaleString()}`,
        },
      },
    },
    scales: {
      x: {
        grid: { color: gridC },
        ticks: { color: textC, font: { size: 10 }, maxTicksLimit: 8, maxRotation: 0 },
      },
      y: {
        grid: { color: gridC },
        ticks: {
          color: textC, font: { size: 10 },
    ...
    <div className="card">
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)', flex: 1, letterSpacing: '-0.2px' }}>
          Sesiones e ingresos — últimos 30 días
        </span>
        <span style={{ fontSize: 11, color: 'var(--ink3)', fontFamily: 'DM Mono, monospace' }}>diario</span>
      </div>

      <div style={{ height: 220, position: 'relative' }}>
        <Line data={data} options={options} aria-label="Gráfico de tendencia de sesiones e ingresos" />
      </div>

      <div style={{ display: 'flex', gap: 16, marginTop: 10, fontSize: 11 }}>
        <Legend color="#2563eb" label="Sesiones" dashed={false} />
        <Legend color="#059669" label="Ingresos (€)" dashed />
      </div>
    </div>
  )
}
